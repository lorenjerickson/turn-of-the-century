...
describe("workspace token interactions", () => {
    ...
    it("activates native wall tools instead of workspace map pointer handlers", () => {
        ...
        assert.match(workspaceRootSource, /this\.sceneDesignFeature\.wallAddSequence = step\.sequence/);
        assert.match(workspaceRootSource, /event\.key === "Escape"[\s\S]*sceneDesignFeature\.cancelWallAddSequence\(\{ notify: false \}\)[\s\S]*wallCommand: "add"/);
        assert.match(workspaceRootSource, /Wall placement reset\. Click to set a new starting point\./);
