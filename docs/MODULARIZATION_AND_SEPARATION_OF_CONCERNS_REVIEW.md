...
Date: 2026-06-21 (last updated 2026-06-25)
...
All eight workspace features, the full encounter resolution engine, and the compendium publication pipeline have been extracted. The remaining opportunities are completing the scene-design repository boundary decoupling and splitting `sample-content.mjs` content categories into per-category files.

| Plan area | Current state |
| --- | --- |
| Workspace feature contract | Complete. The root routes context preparation, binding, panel rendering, and disposal through registered `WorkspaceFeature` instances. |
| Encounter planning | Complete. `EncounterPlanningFeature` is registered by default and owns the player-planner context, rendering, DOM handlers, movement/targeting sessions, canvas listeners, overlays, and disposal. The former root implementation has been removed. |
| Workspace root cleanup | Substantially complete. All eight features are extracted and registered: `EncounterPlanningFeature`, `SceneDesignFeature`, `CampaignFeature`, `MarketFeature`, `RollRequestFeature`, `ActorManagementFeature`, `MediaFeature`, and `WorkspaceLayoutFeature`. `WorkspaceRootApp` is now approximately 1,765 lines (down from 5,684). The compendium preflight side-effect has been moved to `module/bootstrap.mjs`. Remaining gap: `SceneWorkspaceController` is still directly instantiated in the root constructor and passed as a dependency into `SceneDesignFeature` and `WorkspaceLayoutFeature` rather than being hidden behind a repository boundary; some Foundry hook handlers are also wired directly in the root constructor rather than through features. These gaps will close with the repository boundary extraction described in Opportunity 3. |
| Scene design consolidation | Substantially complete. `SceneDesignFeature` now owns wall selection and joinability, toolbar state, detection overlays, wall-placement session state, keyboard binding, grid-calibration input listeners, wall-command canvas listeners, design issue panel context, design-issue hook family (17 Foundry hooks), design issue navigation, design action dispatch (`executeDesignAction`), and disposal. The feature exposes `openScenePropertiesPanel`, `openSceneGridConfiguration`, and `createSceneDesignScene` so design-action implementations can call back through the feature rather than the root. Root delegates all design-action button clicks and navigate-design-issue clicks to the feature. Remaining gap: repository boundary extraction is in progress (uncommitted). `canvas-interaction-adapter.mjs` (26 lines), `scene-repository.mjs` (112 lines), and `wall-repository.mjs` (251 lines) exist and are imported by `SceneDesignFeature`. `SceneWorkspaceController` is still constructor-injected into `SceneDesignFeature` with approximately 20 call sites; full decoupling is not yet complete. |
| Layout feature | Complete. `WorkspaceLayoutFeature` owns dock/stack/floating-window rendering (`renderShell`), drag-and-drop, resize sessions, design lens toggle state, command palette state and wiring, shell menu buttons, and all dock/panel DOM event handlers. Root's `_renderHTML` delegates entirely to `workspaceLayoutFeature.renderShell(context)`. Root's `_onRender` no longer wires any layout DOM handlers (they are bound via the feature loop). ~22 private rendering and interaction methods, 6 state variables, 3 imports, and 5 module-level constants were removed from `workspace-root-app.mjs`. |
| Pure encounter resolution engine | Substantially complete. All modules from the target architecture exist in `module/encounters/`: `EncounterPlanningService`, `EncounterResolutionEngine`, `MovementResolver`, `AttackResolver`, `ReactionResolver`, `CollisionResolver`, `ConsumptionResolver`, `EncounterSnapshotStore`, and `EncounterNarrator`. `combat.mjs` is now approximately 1,430 lines (down from 3,269). `TurnOfTheCenturyEncounter` composes the resolvers; `TurnOfTheCenturyCombat` is the Foundry document subclass. `EncounterResolutionEngine` has no Foundry globals. `EncounterSnapshotStore` correctly uses Foundry API as the adapter layer. Tests are split by module across 20 files in `test/encounters/`. Minor remaining coupling: `EncounterPlanningService` uses `foundry.utils.deepClone` and `game.user?.isGM` directly rather than through injected ports. |
| Content/publication split | Substantially complete. Publication infrastructure has been extracted to `module/compendiums/`: `manifest.mjs` (pack name registry), `legacy-normalizer.mjs` (`migrateLegacyExportSourceData`), `validator.mjs` (`validateStarterCompendiumPlans`, `formatStarterCompendiumPreflightError`), and `publisher.mjs` (`createTotcSampleContent`, `publishTotcSampleCompendiums`). `sample-content.mjs` is now approximately 3,535 lines and contains only declarative content definitions with no Foundry runtime import. Callers in `migrations/` and tests import directly from `compendiums/`. Remaining opportunity: split content categories into per-category files under `module/content/` to reduce the per-item editing context further. |

The repository boundary files (`scene-repository.mjs`, `wall-repository.mjs`, `canvas-interaction-adapter.mjs`) have been created and integrated into `SceneDesignFeature`; that slice is ongoing and uncommitted. The compendium split is complete: `module/compendiums/` now owns all publication infrastructure and `sample-content.mjs` is pure content definitions. The next slice is completing the `SceneWorkspaceController` decoupling in `SceneDesignFeature`, then optionally splitting content categories within `sample-content.mjs`.
